...
is built — one 2.7 MB AOT binary, 30 tests, four universal gates, running this repository's own gate set._
...
- [ ] **DEVKIT1 — `daoris-devkit scan --history`.** The scan covers staged changes, the working tree and
  a commit message; it does **not** cover history. That is the mode the original tool documented as "an
  audit you run at moments, not routinely: before making a repo public, and after any filter-repo scrub
  to prove it worked" — deliberately kept out of `verify`, because its cost grows with the history and
  wiring it into every commit would re-check commits that were already checked when they were made.
  _Found by needing it: the first push on 2026-08-05 required exactly this audit and it was run by hand
  — every commit message, every path that ever existed, and all 932 reachable objects. A one-way door
  with a manual gate in front of it is the argument for building the gate._
