import type { DetectedHarness, Harness } from './types.ts';
 ...
 */
export function resolveHarness(id: string = DEFAULT_HARNESS): Harness {
...
export const tierNames = (harness: Harness): string[] => Object.keys(harness.tiers);
...
export function verifyHarnessContract(
  root: string,
  target: string,
  harness: Harness = HARNESSES[DEFAULT_HARNESS]!,
): string[] {
  const problems = [];
...
export function harnessVerdict(
  root: string,
): { detected: DetectedHarness[]; supported: DetectedHarness[]; others: DetectedHarness[] } {
