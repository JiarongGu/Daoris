import type { CanonNote } from './types.ts';
...
export function compareVersions(a: string | null, b: string | null): number {
  // A missing version sorts before every real one, which is what `from = null` means: nothing
  // synced yet, so every entry is new.
 ...
 * has nothing to add.
 */
export function notesBetween(canonRoot: string, from: string | null, to: string): CanonNote[] {
