 ...
 */
export const STATUSES = ['open', 'taken', 'done', 'declined'];
 ...
 */
export function questId({ from, title, date }) {
 ...
 */
export function renderQuest({ from, title, body, date, status = 'open', note = null, id = null }) {
...
export function readQuests(text) {
 ...
 * they reorganised.
 */
export function planPost({ targetRoot, from, title, body, date }) {
...
export function planStatus({ root, id, status, note, date }) {
...
function parseArgs(argv, valued = ['title', 'body', 'reason']) {
  const flags = {};
  const positional = [];
...
export function commandQuest({ root, argv, write }) {
  ...
  if (verb === 'post') {
    ...
    if (!subject || !title || !body) throw new DaorisError(USAGE);
  ...
  if (['take', 'done', 'decline'].includes(verb)) {
    ...
    if (verb === 'decline' && !flags.reason) {
      throw new DaorisError('declining needs --reason: the reason is the part the asker can act on.');
    }

    const status = verb === 'take' ? 'taken' : verb === 'done' ? 'done' : 'declined';
    const plan = planStatus({
      root, id: subject.replace(/^#/, ''), status, note: flags.reason ?? null, date: today,
    });
