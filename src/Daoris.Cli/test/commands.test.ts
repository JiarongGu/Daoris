...
test('init writes a manifest with no packs and reports what is available', () => {
  ...
  const out: string[] = [];
  assert.equal(commandInit({ root: repoFx.root, write: (s: string) => out.push(s), packageRoot: '' }), 0);
...
test('init refuses to overwrite an existing manifest', () => {
  ...
  const error = captureError(() =>
    commandInit({ root: repoFx.root, write: () => {}, packageRoot: '' }),
  );
...
test('status says when the shipped canon is newer than the lock', () => {
  ...
  const out: string[] = [];
  assert.equal(commandStatus({ root: repoFx.root, write: (s: string) => out.push(s), packageRoot: '' }), 0);
  const text = out.join('\n');
